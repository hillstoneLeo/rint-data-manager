...
def handle_dvc_upload_data_item_creation(file_path: str, full_path: Path,
                                         user: User,
                                         db: Session) -> Optional[DataItem]:
    ...
    try:
        ...
        if is_dvc_hash_path(file_path):
            ...
            if not metadata['original_filename'] and full_path.exists():
                stat = full_path.stat()
...
def get_file_path_from_hash(file_hash: str,
                            user_email: Optional[str] = None) -> Path:
...
async def upload_dvc_file(file_path: str,
                          request: Request,
                          authorization: Optional[str] = Header(None),
                          x_dvc_token: Optional[str] = Header(None),
                          db: Session = Depends(get_db)):
    ...
    if user:
        try:
            handle_dvc_upload_data_item_creation(file_path, full_path, user,
                                                 db)
        except Exception as e:
            # Log error but don't fail the upload
            print(f"Error creating DataItem record: {e}")

    return DVCUploadResponse(status="success",
                             path=file_path,
                             user_email=str(user.email) if user else None)


@router.post("/{file_path:path}")
async def post_dvc_file(file_path: str,
                        request: Request,
                        authorization: Optional[str] = Header(None),
                        x_dvc_token: Optional[str] = Header(None),
                        db: Session = Depends(get_db)):
    ...
    return await upload_dvc_file(file_path, request, authorization,
                                 x_dvc_token, db)


@router.head("/{file_path:path}")
async def head_dvc_file(file_path: str,
                        authorization: Optional[str] = Header(None),
                        x_dvc_token: Optional[str] = Header(None),
                        db: Session = Depends(get_db)):
    ...
    stat = full_path.stat()
    return DVCFileResponse(path=file_path,
                           size=stat.st_size,
