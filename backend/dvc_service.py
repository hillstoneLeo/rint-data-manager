...
async def create_folder_data_item(
    ...
    for file in files:
        ...
        file_subdir = folder_path / normalized_path.rsplit('/', 1)[0] if '/' in normalized_path else folder_path
    ...
    try:
        ...
        relative_folder_path = os.path.relpath(str(folder_path), DVC_UPLOADS_PROJECT)
        subprocess.run(["dvc", "add", relative_folder_path], check=True, capture_output=True, cwd=DVC_UPLOADS_PROJECT)
...
async def create_data_item(
    ...
    if '/' in file_name or '\\' in file_name:
        # Normalize path separators and create subdirectories
        normalized_path = file_name.replace('\\', '/')
        file_subdir = user_upload_dir / normalized_path.rsplit('/', 1)[0] if '/' in normalized_path else user_upload_dir
        file_subdir.mkdir(parents=True, exist_ok=True)
